import { Button, Card, Col, notification, Row, Spin, Statistic, Table, Tag } from 'antd';
...
function SageDebug() {
  ...
  const getAccountDetail = async (address: string) => {
    try {
      console.log("sageContract:" + sageContract.options.address);
  ...
  const buyLevel = async (level: number) => {
    //buyNewLevel
    try {
      console.info({
        "message": "buyLevel",
      ...
      const price = sageLevelPrices[level];
      ...
      sageContract.methods
        ...
        .once('receipt', () => {
          notification.info({
            ...
            description: (
              <span>
                <span>page will refresh after </span>
                <Statistic.Countdown
                  format="s[S]"
              ...
              <div key={index}>
                <div
                  ...
                  <Card.Grid style={{
                    ...
                    {sageDataSource ? sageDataSource[index].active ? (<p>acitived</p>) : (<Button onClick={(event) => { buyLevel(index) }}>Buy</Button>) : (<Button>Buy</Button>)}
                  </Card.Grid>
                  ...
                  <Card.Grid style={{
                    ...
                    height: 40,
                    padding: 4
                  }}>blocked:{sageDataSource ? sageDataSource[index].blcoked : false}</Card.Grid>
                  <Card.Grid style={{
                    width: '25%',
                    textAlign: 'center',
                    height: 40,
                    padding: 4
                  }}>reopen:{sageDataSource ? sageDataSource[index].x3reopen : false}</Card.Grid>
